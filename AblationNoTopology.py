"""
Ablation Study 3: No Small-World Topology
==========================================
Flat list storage instead of Watts-Strogatz graph.
Tests: what does the small-world topology contribute vs flat retrieval?

Run: python ablation_no_topology.py
"""
...
class FlatStore:
    """
    KEY CHANGE: No graph topology at all.
    Just a flat list with token_value sorting and hub promotion.
    Tests what small-world topology contributes vs flat retrieval.
    """
    def __init__(self):
        self.nodes = {}   # flat dict, no graph
        self.hubs = set()
        self.n_id = 0
    ...
    def read(self, query_type, limit=3):
        # Flat scan — O(N) instead of O(log N)
        candidates = [(nid, node) for nid, node in self.nodes.items()
                      if node["type"] == query_type or nid in self.hubs]
        # Sort by token_value (economy preserved, topology removed)
        candidates.sort(key=lambda x: (x[0] in self.hubs, x[1]["token_value"]), reverse=True)
...
def run_ablation_no_topology(n_agents, n_turns):
    print(f"\n{'='*60}")
    print("ABLATION: FLAT STORE (no small-world topology)")
    print(f"{'='*60}")

    ledger = TokenLedger("no_topology")
    store = FlatStore()
    ...
    for turn in range(n_turns):
        ...
        if turn > 0 and turn % EPOCH_SIZE == 0:
            store.promote_hubs()
            print(f"  [Epoch {epoch}] Hubs: {len(store.hubs)}, nodes={len(store.nodes)}")

        for agent_id in range(n_agents):
            role = AGENT_ROLES[agent_id % len(AGENT_ROLES)]
            relevant_nodes = store.read("insight", limit=3)
            context_parts = [c for _, c in relevant_nodes]
            context_str = "\n".join(context_parts) if context_parts else "No prior context yet."

            graph_query_tokens = store.context_tokens_for("insight")
            ...
            messages = [{
                ...
                "content": (
                    ...
                    f"Relevant prior knowledge (from flat store):\n{context_str}\n\n"
            ...
            store.write(output, "insight", agent_id, output_tokens, epoch)

            print(f"  [FlatStore] Agent {agent_id} Turn {turn+1}: "
                  f"{ledger.records[-1]['prompt_tokens']}+{ledger.records[-1]['completion_tokens']} tokens "
                  f"(cumulative: {ledger.total:,}) | hubs={len(store.hubs)} nodes={len(store.nodes)}")
...
def save_results(ledger, filename):
    ...
    print(f"\nABLATION 3 RESULTS (No Small-World Topology)")
    print(f"Total tokens: {ledger.total:,}")
    print(f"CSV saved: {path}")
    return rows

if __name__ == "__main__":
    print("Ablation Study 3: No Small-World Topology (flat store)")
    print(f"Config: {N_AGENTS} agents, {N_TURNS} turns")
    ledger = run_ablation_no_topology(N_AGENTS, N_TURNS)
    save_results(ledger, "ablation_no_topology.csv")
